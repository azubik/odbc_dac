...
// |<-------------------- FRecBufSize --------------------->|
  ...
  protected
    ...
    procedure ApplyCachedToDBMS(UpdateKind: TUpdateKind; Buffer: TRecordBuffer);
...
procedure TODBCCustomDataSet.ApplyCachedToDBMS(UpdateKind: TUpdateKind; Buffer: TRecordBuffer);
begin
  with PRecInfo(Buffer)^ do
    case UpdateKind of
      ukDelete:
...
begin
  ...
  begin
    ...
    begin
      ...
      Move(Buffer[0], Buff[FMetaFields^[Field.FieldNo].DataOfs], FMetaFields^[Field.FieldNo].DataSize);
    end;
    SetModified(True);
  end
  else
  begin
    ...
    if Boolean(TmpBuff[0]) then
      Move(Buffer[0], TmpBuff[1], Field.DataSize);
  end;
...
procedure TODBCCustomDataSet.InternalAddRecord(Buffer: TRecordBuffer; Append: Boolean);
begin
  CheckEditState;
  CursorPosChanged;
...
function TODBCCustomDataSet.GetFieldData(Field: TField; Buffer: TValueBuffer): Boolean;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if Result and (Buffer <> nil) then
        Move(Buff[1], Buffer[0], Field.DataSize);
    end;
  end;
end;

function TODBCCustomDataSet.GetFieldData(FieldNo: Integer; Buffer: TValueBuffer): Boolean;
begin
