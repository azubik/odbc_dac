...
function ClearBcdThousandSeparators(const ARegionalBcdStr: string): string;
begin
  Result := StringReplace(ARegionalBcdStr, {$IFDEF RTLVersion >= 16.0}FormatSettings.{$ENDIF}ThousandSeparator, '', [rfReplaceAll]);
end;

function ReplaceRegionalDecimalSeparator(const ARegionalBcdStr: string): string;
begin
  Result := StringReplace(ARegionalBcdStr, {$IFDEF RTLVersion >= 16.0}FormatSettings.{$ENDIF}DecimalSeparator, SQL_DECIMAL_SEPARATOR, [rfReplaceAll]);
end;
