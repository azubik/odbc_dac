...
type
  ...
  private
    ...
    FCurrentIdxFlds: TList<TField>;                       // Current Index Fields
  ...
  protected
    ...
    procedure DataEvent(Event: TDataEvent; Info: {$IF RTLVersion >= 16.0}NativeInt{$ELSE}Longint{$IFEND}); override;
...
begin
  ...
  FCurrentIdxFlds := TList<TField>.Create;
...
begin
  ...
  try
    GetFieldList(TList<TField>(nil), IdxDef.Fields);
  except
    IdxDef := nil;
  end;
...
begin
  ...
  with PRecInfo(PByte(Buffer) + SizeOf(TKeyBuffer))^ do
  begin
...
procedure TODBCTable.SetLinkRanges(MasterFields: TList<TField>);
var
  SaveState: TDataSetState;
  i: Integer;
begin
...
begin
  ...
  begin
    ...
    Result := QuoteStr(SchemaNamePart, Char(FQuoteChar)) + SCHEMA_SEPARATOR + QuoteStr(TableNamePart, FQuoteChar);
  end;
end;
...
{$IFNDEF USE_UNICODE_DRIVER}
  IdxNameA, ColNameA: array[0..STR_LEN - 1] of SQLCHAR;
{$ELSE}
  IdxNameW, ColNameW: array[0..STR_LEN - 1] of SQLWCHAR;
{$ENDIF}
  ...
  info_type: SmallInt;
{$IFNDEF USE_UNICODE_DRIVER}
  asc_or_descA: array[0..1] of SQLCHAR;
{$ELSE}
  asc_or_descW: array[0..1] of SQLWCHAR;
{$ENDIF}
...
begin
  ...
  try
    ...
    try
{$IFNDEF USE_UNICODE_DRIVER}
      RetCode := SQLStatisticsA(TmpHandle, nil, 0, PSQLCHAR(SchemaNamePart), SQL_NTS {0}, PSQLCHAR(AnsiString(TableNamePart)), SQL_NTS, SQL_INDEX_ALL, SQL_QUICK);
{$ELSE}
      RetCode := SQLStatisticsW(TmpHandle, nil, 0, PSQLWCHAR(SchemaNamePart), SQL_NTS {0}, PSQLWCHAR(TableNamePart), SQL_NTS, SQL_INDEX_ALL, SQL_QUICK);
{$ENDIF}
      if not SQL_SUCCEEDED(RetCode) then
      ...
      // index name
{$IFNDEF USE_UNICODE_DRIVER}
      RetCode := SQLBindCol(TmpHandle, 6, SQL_C_CHAR, @IdxNameA, STR_LEN, @SL_Ind);
{$ELSE}
      RetCode := SQLBindCol(TmpHandle, 6, SQL_C_WCHAR, @IdxNameW, STR_LEN, @SL_Ind);
{$ENDIF}
      if not SQL_SUCCEEDED(RetCode) then
        ODBCError(SQL_HANDLE_STMT, TmpHandle, RetCode);
      ...
      // column name
{$IFNDEF USE_UNICODE_DRIVER}
      RetCode := SQLBindCol(TmpHandle, 9, SQL_C_CHAR, @ColNameA, STR_LEN, @SL_Ind);
{$ELSE}
      RetCode := SQLBindCol(TmpHandle, 9, SQL_C_WCHAR, @ColNameW, STR_LEN, @SL_Ind);
{$ENDIF}
      if not SQL_SUCCEEDED(RetCode) then
        ODBCError(SQL_HANDLE_STMT, TmpHandle, RetCode);
      ...
      // asc or desc
{$IFNDEF USE_UNICODE_DRIVER}
      RetCode := SQLBindCol(TmpHandle, 10, SQL_C_CHAR, @asc_or_descA, SizeOf(asc_or_descA), @SL_Ind);
{$ELSE}
      RetCode := SQLBindCol(TmpHandle, 10, SQL_C_WCHAR, @asc_or_descW, SizeOf(asc_or_descW), @SL_Ind);
{$ENDIF}
      if not SQL_SUCCEEDED(RetCode) then
        ODBCError(SQL_HANDLE_STMT, TmpHandle, RetCode);
      ...
      begin
        case info_type of
          SQL_INDEX_CLUSTERED, SQL_INDEX_HASHED, SQL_INDEX_OTHER:
            begin
{$IFNDEF USE_UNICODE_DRIVER}
              IdxName := UnicodeString(AnsiString(PSQLCHAR(@IdxNameA)));
              ColName := UnicodeString(AnsiString(PSQLCHAR(@ColNameA)));
{$ELSE}
              IdxName := UnicodeString(PSQLWCHAR(@IdxNameW));
              ColName := UnicodeString(PSQLWCHAR(@ColNameW));
{$ENDIF}
...
{$IFNDEF USE_UNICODE_DRIVER}
                if asc_or_descA[0] = SQLCHAR('D') then      // do not localize
{$ELSE}
                if asc_or_descW[0] = SQLWCHAR('D') then      // do not localize
{$ENDIF}
...
{$ENDIF}
      ...
      // get primary key
{$IFNDEF USE_UNICODE_DRIVER}
      RetCode := SQLPrimaryKeysA(TmpHandle, nil, 0, PSQLCHAR(SchemaNamePart) {nil}, SQL_NTS {0}, PSQLCHAR(TableNamePart), SQL_NTS);
{$ELSE}
      RetCode := SQLPrimaryKeysW(TmpHandle, nil, 0, PSQLWCHAR(SchemaNamePart) {nil}, SQL_NTS {0}, PSQLWCHAR(TableNamePart), SQL_NTS);
{$ENDIF}
      ...
      begin
        // column name
{$IFNDEF USE_UNICODE_DRIVER}
        RetCode := SQLBindCol(TmpHandle, 4, SQL_C_CHAR, @ColNameA, STR_LEN, @SL_Ind);
{$ELSE}
        RetCode := SQLBindCol(TmpHandle, 4, SQL_C_WCHAR, @ColNameW, STR_LEN, @SL_Ind);
{$ENDIF}
        if not SQL_SUCCEEDED(RetCode) then
          ODBCError(SQL_HANDLE_STMT, TmpHandle, RetCode);
...
procedure TODBCTable.DataEvent(Event: TDataEvent; Info: {$IF RTLVersion >= 16.0}NativeInt{$ELSE}Longint{$IFEND});
begin
...
begin
  ...
  begin
    ...
    try
      ...
      //TmpQuery.SQL.Text := Format('SELECT * FROM %s WHERE false', [QuoteTableName]); // do not localize
      ...
      try
        ...
        for i := 1 to TmpQuery.Cursor.Columns.Count - 1 do // ignore Column[0] - reserved for ODBC bookmarks
          ...
          begin
            ...
            Size := GetFieldDefSize(DataType, TmpCol.ColumnSize, TmpCol.DataLength, TmpCol.DecimalDigits);
...
{$IFDEF NEXTGEN}
    _OldBuffer := AllocRecBuf;
{$ELSE}
    _OldBuffer := TRecBuf(AllocRecordBuffer);
{$ENDIF NEXTGEN}
...
procedure TODBCTable.SetFieldData(Field: TField; Buffer: TValueBuffer);
var
  Buff: PByte;
begin
  ...
  begin
    ...
    Buff := PByte(FKeyBuffer) + SizeOf(TKeyBuffer);
    ...
    begin
      ...
      Move(Buffer[0], Buff[MetaFields^[Field.FieldNo].DataOfs], MetaFields^[Field.FieldNo].DataSize);
    end;
    SetModified(True);
  end
  else
...
procedure TODBCTable.GetDetailLinkFields(MasterFields, DetailFields: TList<TField>);
var
  Idx: TIndexDef;
  i: Integer;
begin
...
function TODBCTable.GetFieldData(Field: TField; var Buffer: TValueBuffer): Boolean;
var
  Buff, Data: PByte;
begin
  if State = dsSetKey then
  begin
    ...
    begin
      Data := PByte(Buff) + MetaFields^[Field.FieldNo].DataOfs;
      Move(Data^, Buffer[0], Field.DataSize);
    end;
  end
  else
