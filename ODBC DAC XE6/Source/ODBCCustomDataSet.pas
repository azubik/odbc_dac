...
// |<-------------------- FRecBufSize --------------------->|
  ...
  private
    ...
    FChunkSize: Integer;
    FChunkCount: Integer;
    FCurrentChunk: TRecBuf;

    FCacheList: TList<TRecBuf>;
    ...
    procedure FreeBlobFields(Buffer: TRecBuf);

    procedure FetchNext(Buffer: TRecBuf);
    function WriteBufferToCache(ARecNum: Integer; Buffer: TRecBuf): TRecBuf;

    procedure CopyBuffer(Source: TRecBuf; Dest: TRecBuf);
    procedure SaveToOldCache(Buffer: TRecBuf);

    function GetCacheBuffer(Buffer: TRecBuf): TRecBuf;
    function GetCacheBufferA(ARecNum: Integer): TRecBuf;
    function GetCacheOldBuffer(ARecNum: Integer): TRecBuf;
    ...
    procedure GetBlobData(Field: TField; Buffer: TRecBuf; out Data: Pointer; out DataLength: Integer);
    procedure SetBlobData(Field: TField; Buffer: TRecBuf; Data: Pointer; DataLength: Integer);

    procedure AddToUpdateHistory(Buffer: TRecBuf);
    procedure RemoveFromUpdateHistory(Buffer: TRecBuf);
  ...
  protected
    ...
    function GetActiveBuffer: TRecBuf;
    ...
    procedure ApplyCachedToDBMS(UpdateKind: TUpdateKind; Buffer: TRecBuf);

    procedure ApplyCachedUpdate(Buffer: TRecBuf; var UpdateAction: TUpdateAction); virtual;
    procedure RevertCachedUpdate(Buffer: TRecBuf);
...
{$ENDIF NEXTGEN}
    ...
    property _CurrentChunk: TRecBuf read FCurrentChunk write FCurrentChunk;

    property _CacheList: TList<TRecBuf> read FCacheList write FCacheList;

    property _OldCache: TRecBuf read FOldCache write FOldCache;
    ...
    property _UpdateHistory: TList<TRecBuf> read FUpdateHistory write FUpdateHistory;

    property _FilterBuffer: TRecBuf read FFilterBuffer write FFilterBuffer;
    property _OldBuffer: TRecBuf read FOldBuffer write FOldBuffer;
...
{$ENDIF}
  ...
  private
    ...
    procedure SetInternalSQLParams(Stmt: TODBCCustomStatement; Buffer: TRecBuf);
  ...
  protected
    ...
    procedure ApplyCachedUpdate(Buffer: TRecBuf; var UpdateAction: TUpdateAction); override;
...
begin
  ...
  FCacheList := TList<TRecBuf>.Create;
  FUpdateHistory := TList<TRecBuf>.Create;
...
procedure TODBCCustomDataSet.FreeBlobFields(Buffer: TRecBuf);
var
  i: Integer;
begin
  if FBlobBufferSize > 0 then
    for i := 0 to FQStmt.Cursor.BlobColumnCount - 1 do
      SetLength(PBlobDataArray(PByte(Buffer) + FBlobCacheOfs)^[i], 0);
end;

procedure TODBCCustomDataSet.FetchNext(Buffer: TRecBuf);
var
  Fld: TField;
...
begin
  ...
  if FBlobBufferSize > 0 then
    FillChar(PByte(Buffer)[FBlobCacheOfs], FBlobBufferSize, 0);
  ...
  begin
    ...
    begin
      PByte(Buffer)[FieldDesc.NullFlagOfs] := FieldIsNull;
      if StmtField.IsBlob and FCacheBlobs then
        Inc(FieldOffset);
    end
    else
    begin
      PByte(Buffer)[FieldDesc.NullFlagOfs] := FieldIsNotNull;

      case StmtField.SQLType of
        SQL_CHAR, SQL_VARCHAR,
          SQL_INTERVAL, SQL_INTERVAL_YEAR..SQL_INTERVAL_MINUTE_TO_SECOND:
          begin
            Move(StmtField.Data^, PByte(Buffer)[FieldDesc.DataOfs], StmtField.DataLength);
          end;
        ...
        SQL_SMALLINT, SQL_TINYINT,
          ...
          SQL_BIGINT,
          SQL_BIT:
          Move(StmtField.Data^, PByte(Buffer)[FieldDesc.DataOfs], StmtField.DataLength {FieldDesc^.DataSize});

        SQL_DECIMAL, SQL_NUMERIC:
          PBcd(@(PByte(Buffer)[FieldDesc.DataOfs]))^ := StmtField.AsBcd;

        SQL_TYPE_DATE:
          PSQLINTEGER(@(PByte(Buffer)[FieldDesc.DataOfs]))^ := DateTimeToTimeStamp(StmtField.AsDateTime).Date;

        SQL_TYPE_TIME:
          PSQLINTEGER(@(PByte(Buffer)[FieldDesc.DataOfs]))^ := DateTimeToTimeStamp(StmtField.AsDateTime).Time;

        SQL_TYPE_TIMESTAMP:
          PSQLDOUBLE(@(PByte(Buffer)[FieldDesc.DataOfs]))^ := TimeStampToMSecs(DateTimeToTimeStamp(StmtField.AsDateTime));

        SQL_BINARY, SQL_VARBINARY, SQL_LONGVARBINARY,
        SQL_LONGVARCHAR, SQL_WLONGVARCHAR:
          if FCacheBlobs then
          begin
            SetLength(PBlobDataArray(PByte(Buffer) + FBlobCacheOfs)^[FieldOffset], StmtField.DataLength);
            Move(StmtField.Data^, PByte(PBlobDataArray(PByte(Buffer) + FBlobCacheOfs)^[FieldOffset])^, StmtField.DataLength);
            Inc(FieldOffset);
          end;

        SQL_GUID:
          Move(PByte(StmtField.Data)^, PByte(Buffer)[FieldDesc.DataOfs], StmtField.DataLength);
      else
...
function TODBCCustomDataSet.WriteBufferToCache(ARecNum: Integer; Buffer: TRecBuf): TRecBuf;
var
  Index: Integer;
begin
  ...
  begin
    ...
    GetMem(PByte(FChunks[Index]), FChunkSize);
    FCurrentChunk := FChunks[Index];
  end;

  Result := FCurrentChunk + (ARecNum mod FBufferChunks) * FRecBufSize;
  Move(PByte(Buffer)^, PByte(Result)^, FRecBufSize);
end;

procedure TODBCCustomDataSet.CopyBuffer(Source: TRecBuf; Dest: TRecBuf);
var
  i: Integer;
begin
  FreeBlobFields(Dest);

  Move(PByte(Source)^, PByte(Dest)^, FRecBufSize);
  FillChar(PByte(Dest)[FBlobCacheOfs], FBlobBufferSize, 0);
...
procedure TODBCCustomDataSet.SaveToOldCache(Buffer: TRecBuf);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      ReallocMem(PByte(FOldCache), FOldCacheSize);
    end;

    Move(PByte(Buffer)[0], PByte(FOldCache)[OldCacheOfs], FRecBufSize);
    Inc(FOldRecordCount);
  end;
end;

function TODBCCustomDataSet.GetCacheBuffer(Buffer: TRecBuf): TRecBuf;
begin
  Result := GetCacheBufferA(PRecInfo(Buffer)^.PhyRecNum);
end;

function TODBCCustomDataSet.GetCacheBufferA(ARecNum: Integer): TRecBuf;
var
  i: Integer;
begin
  i := 0;
...
function TODBCCustomDataSet.GetCacheOldBuffer(ARecNum: Integer): TRecBuf;
var
  Size: Integer;
  Offset: Integer;
begin
...
function TODBCCustomDataSet.AdjustCurrentRecord(Buffer: TRecBuf; GetMode: TGetMode): TGetResult;
begin
  Result := grOK;
  ...
  while not IsVisible(Buffer) do
    ...
    begin
      ...
      if FRecordPos >= 0 then
        Move(PByte(FCacheList[FRecordPos])^, PByte(Buffer)^, FRecBufSize)
      else
      begin
        Result := grBOF;
        Break;
...
function TODBCCustomDataSet.IsVisible(Buffer: TRecBuf): Boolean;
begin
  Result := True;
...
procedure TODBCCustomDataSet.GetBlobData(Field: TField; Buffer: TRecBuf; out Data: Pointer; out DataLength: Integer);
begin
  Data := PBlobDataArray(PByte(Buffer) + FBlobCacheOfs)^[Field.Offset];
  DataLength := Length(PBlobDataArray(PByte(Buffer) + FBlobCacheOfs)^[Field.Offset]);
end;

procedure TODBCCustomDataSet.SetBlobData(Field: TField; Buffer: TRecBuf; Data: Pointer; DataLength: Integer);
var
  BlobData: TBytes;
begin
  if Buffer <> ActiveBuffer then Exit;

  BlobData := PBlobDataArray(PByte(Buffer) + FBlobCacheOfs)^[Field.Offset];

  SetLength(BlobData, 0);               // clear old blobdata
  SetLength(BlobData, DataLength);      // get new memory
  Move(Data^, PByte(BlobData)^, DataLength);

  PBlobDataArray(PByte(Buffer) + FBlobCacheOfs)^[Field.Offset] := BlobData;
end;

procedure TODBCCustomDataSet.AddToUpdateHistory(Buffer: TRecBuf);
begin
  if FUpdateHistory.IndexOf(Buffer) = -1 then
    FUpdateHistory.Add(Buffer);
end;

procedure TODBCCustomDataSet.RemoveFromUpdateHistory(Buffer: TRecBuf);
begin
...
begin
  ...
  for i := 0 to Length(FChunks) - 1 do
    FreeMem(PByte(FChunks[i]), FChunkSize);
...
begin
  ...
  if FBlobBufferSize > 0 then
    ...
    begin
      ...
      Inc(PByte(FOldCache), FRecBufSize);
    end;

  // free old cache page
  if FOldCacheSize > 0 then
    FreeMem(PByte(FOldCache), FOldCacheSize);
...
function TODBCCustomDataSet.GetActiveBuffer: TRecBuf;
begin
  Result := 0;
    ...
    case UpdateKind of
      ...
      ukModify:
        begin
          ...
          Move(PByte(TmpBuff)^, PByte(FCacheList[FRecordPos])^, FRecBufSize);
      ...
      ukDelete:
        ...
        begin
          ...
          begin
            ...
            Move(PByte(Buff)^, PByte(FCacheList[FRecordPos])^, FRecBufSize);
...
procedure TODBCCustomDataSet.ApplyCachedToDBMS(UpdateKind: TUpdateKind; Buffer: TRecBuf);
begin
  with PRecInfo(Buffer)^ do
    case UpdateKind of
      ukDelete:
...
procedure TODBCCustomDataSet.ApplyCachedUpdate(Buffer: TRecBuf; var UpdateAction: TUpdateAction);
var
...
begin
  ...
  Move(PByte(Buffer)^, PByte(ActiveBuffer)^, FRecBufSize);
...
procedure TODBCCustomDataSet.RevertCachedUpdate(Buffer: TRecBuf);
var
  OldBuff: TRecBuf;
begin
...
{$IFDEF NEXTGEN}
function TODBCCustomDataSet.AllocRecordBuffer: TRecBuf;
begin
  Result := AllocMem(FRecBufSize);
end;

procedure TODBCCustomDataSet.FreeRecordBuffer(var Buffer: TRecBuf);
begin
  FreeMem(Buffer);
end;
{$ELSE}
...
procedure TODBCCustomDataSet.InternalInitRecord(Buffer: TRecBuf);
begin
  FillChar(PByte(Buffer)^, FRecBufSize, 0);
  with PRecInfo(Buffer)^ do
  begin
    UpdateStatus := usInserted;
    PhyRecNum := -1;
...
function TODBCCustomDataSet.InternalGetRecord(Buffer: TRecBuf; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
begin
  Result := grOK;
...
function TODBCCustomDataSet.GetRecord(Buffer: TRecBuf; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
var
  Accept: Boolean;
...
procedure TODBCCustomDataSet.SetFieldData(Field: TField; Buffer: TValueBuffer);
var
  Buff, TmpBuff: TRecBuf;
begin
  Buff := GetActiveBuffer;
  ...
  begin
    ...
    if Buffer = nil then
      PByte(Buff)[FMetaFields^[Field.FieldNo].NullFlagOfs] := FieldIsNull
    else
    begin
      PByte(Buff)[FMetaFields^[Field.FieldNo].NullFlagOfs] := FieldIsNotNull;
      Move(Buffer[0], PByte(Buff)[FMetaFields^[Field.FieldNo].DataOfs], FMetaFields^[Field.FieldNo].DataSize);
    end;
    SetModified(True);
  end
  else
  begin
    // calculated fields
    PByte(TmpBuff) := PByte(Buff) + FCalcFieldsOfs + Field.Offset;
    Boolean(PByte(TmpBuff)[0]) := LongBool(Buffer);

    if Boolean(PByte(TmpBuff)[0]) then
      Move(Buffer[0], PByte(TmpBuff)[1], Field.DataSize);
  end;
...
function TODBCCustomDataSet.GetBookmarkFlag(Buffer: TRecBuf): TBookmarkFlag;
begin
  Result := PRecInfo(Buffer)^.BookmarkFlag;
end;

procedure TODBCCustomDataSet.SetBookmarkFlag(Buffer: TRecBuf; Value: TBookmarkFlag);
begin
  PRecInfo(Buffer)^.BookmarkFlag := Value;
end;

procedure TODBCCustomDataSet.GetBookmarkData(Buffer: TRecBuf; Data: TBookmark);
begin
  PSQLINTEGER(Data)^ := PRecInfo(Buffer)^.PhyRecNum;
end;

procedure TODBCCustomDataSet.SetBookmarkData(Buffer: TRecBuf; Data: TBookmark);
begin
...
procedure TODBCCustomDataSet.InternalSetToRecord(Buffer: TRecBuf);
begin
...
procedure TODBCCustomDataSet.InternalAddRecord(Buffer: TRecBuf; Append: Boolean);
begin
  CheckEditState;
  CursorPosChanged;
...
begin
  ...
  if (Buff <> 0) and (State = dsEdit) then
  begin
...
begin
  ...
  if {DefaultFields} (FieldOptions.AutoCreateMode <> acExclusive) or not (lcPersistent in Fields.LifeCycles) then
    CreateFields;
  BindFields(True);
...
procedure TODBCCustomDataSet.ClearCalcFields(Buffer: TRecBuf);
begin
  FillChar(PByte(Buffer)[FCalcFieldsOfs], CalcFieldsSize, 0);
end;
...
function TODBCCustomDataSet.GetCurrentRecord(Buffer: TRecBuf): Boolean;
begin
...
function TODBCCustomDataSet.GetFieldData(Field: TField; var Buffer: TValueBuffer): Boolean;
var
  Buff, Data: TRecBuf;
begin
  Result := False;
  ...
  begin
    if Field.FieldKind = fkData then
    begin
      Result := PByte(Buff)[FMetaFields^[Field.FieldNo].NullFlagOfs] <> FieldIsNull;
      if Result and (Buffer <> nil) then
      begin
        PByte(Data) := PByte(Buff) + FMetaFields^[Field.FieldNo].DataOfs;
        Move(PByte(Data)^, Buffer[0], Field.DataSize);
      end;
    end
    else
    begin
      // calculated fields
      Inc(PByte(Buff), FCalcFieldsOfs + Field.Offset);
      Result := Boolean(PByte(Buff)[0]);
      if Result and (Buffer <> nil) then
        Move(PByte(Buff)[1], Buffer[0], Field.DataSize);
    end;
  end;
end;

function TODBCCustomDataSet.GetFieldData(FieldNo: Integer; var Buffer: TValueBuffer): Boolean;
begin
...
procedure TODBCInternalDataSet.SetInternalSQLParams(Stmt: TODBCCustomStatement; Buffer: TRecBuf);
var
  Param: TODBCParam;
  ParamName: string;
  TmpField: TField;
...
procedure TODBCInternalDataSet.ApplyCachedUpdate(Buffer: TRecBuf; var UpdateAction: TUpdateAction);
var
  UpdateKind: TUpdateKind;
begin
