...
type
  ...
  private
    ...
    procedure SetAsWideMemo(const Value: WideString);
  ...
  public
    ...
    property AsWideMemo: WideString read GetAsWideString write SetAsWideMemo;
...
{$ELSE}
        Result := ODBCUtils.StrToBcd(ClearBcdThousandSeparators(AsString));
{$ENDIF}
...
{$ELSE}
        Result := ODBCUtils.DoubleToBcd(PSQLDOUBLE(FData)^);
{$ENDIF}
...
{$ELSE}
        Result := ODBCUtils.IntegerToBcd(PSQLINTEGER(FData)^);
{$ENDIF}
    else
...
begin
  ...
  if not IsNull then
    case FCType of
      SQL_C_CHAR:
        try
          Result := StringToGUID(PChar(FData));
        except
          DatabaseError(SInvalidDataConversion);
        end;
...
begin
  ...
  if not IsNull then
    case FCType of
      ...
      SQL_C_WCHAR:
        Result := WideCharToString(PWideChar(FData));
      ...
      SQL_C_BINARY:
        ...
        begin
          ...
          Move(PChar(FData)^, PChar(Result)^, FDataLength);
        end;
...
function TODBCSQLVAR.GetAsWideString: WideString;
begin
  Result := '';
  if not IsNull then
    case FCType of
...
{$ELSE}
  RegionalBcdStr := ODBCUtils.BcdToStr(Value);
{$ENDIF}
  BcdStr := ReplaceRegionalDecimalSeparator(RegionalBcdStr);

  FDataLength := Length(BcdStr);
  FDataBufSize := FDataLength + 1;                // null terminator
  GetMem(FData, FDataBufSize);
  Move(PChar(BcdStr)^, FData^, FDataBufSize);
...
procedure TODBCSQLVAR.SetAsBlob(const Value: TBlobData);
begin
  Clear;
  ...
  GetMem(FData, FDataBufSize);
  Move(PChar(Value)^, FData^, FDataBufSize);
...
procedure TODBCSQLVAR.SetAsMemo(const Value: string);
begin
  Clear;
...
procedure TODBCSQLVAR.SetAsWideString(const Value: WideString);
begin
  Clear;
  ...
  GetMem(FData, FDataBufSize);
  Move(PWideChar(Value)^, FData^, FDataBufSize);
...
procedure TODBCSQLVAR.SetAsWideMemo(const Value: WideString);
begin
  Clear;
...
begin
  ...
  case BlobType of
    ...
    ftMemo:
      SetAsMemo(DataStr);
    ftFmtMemo:
      SetAsWideMemo(PWideChar(Pointer(DataStr)));
  else
  ...
  begin
    if VarIsNull(Field.OldValue) then
      ...
      {$IFDEF VCL50}
            ...
            begin
              CurrToBcd(TBCDField(Field).OldValue, BcdVal);
              SetAsBCD(BcdVal);
            end
            else
      {$ENDIF}
  ...
  begin
    if Field.IsNull then
      ...
      {$IFDEF VCL50}
            if Field.DataType = ftBCD then
            begin
              CurrToBcd(TBCDField(Field).Value, BcdVal);
              SetAsBCD(BcdVal);
            end
            else
      {$ENDIF}
...
{$ELSE}
        ColumnSize := TBCDField(Field).Precision;
{$ENDIF}
...
procedure TODBCCursor.DescribeColumns;
  ...
  begin
    case SQLType of
      SQL_CHAR, SQL_VARCHAR:
        Result := ColDataLen + SizeOf(Char);      // terminated 0
...
var
  Column: TODBCColumn;
  ColName: array[0..STR_LEN - 1] of SQLCHAR;
...
begin
  ...
  begin
    CheckError(SQLDescribeCol(FHandle, i, @ColName, STR_LEN, @ColNameLength, @SQLType, @ColumnSize, @DecimalDigits, @Nullable));
    CheckError(SQLColAttribute(FHandle, i, SQL_DESC_AUTO_UNIQUE_VALUE, nil, 0, nil, @NumAttr));
      ...
      case Column.CType of
        SQL_C_CHAR:
          repeat
            ...
            RetCode := SQLGetData(FHandle, Column.Index, Column.CType, PChar(Column.FData) + DataOfs, BLOBCHUNKSIZE, @(Column.FStrLen_Or_Ind));
            CheckError(RetCode);
...
{$IFDEF USE_DRV_SPEC}
      case ODBCDriverType of
        ...
        // DataDirect driver for ParadoxFile
        (*
                drvtDataDirectParadoxFile:
                  case Param.SQLType of
                    // TIMESTAMP data size adjust
                    SQL_INTEGER:
                      begin
...
begin
  ...
  CheckError(SQLSetStmtAttr(FStmtHandle, SQL_ATTR_CURSOR_TYPE, Pointer(SQL_CURSOR_STATIC), SQL_IS_UINTEGER));
  // prepare SQL
  CheckError(SQLPrepare(FStmtHandle, PSQLCHAR(FProcessedSQL), SQL_NTS));
end;
...
begin
  CheckPrepared;
  CheckError(SQLGetStmtAttr(FStmtHandle, Attribute, ValuePtr, BufferLength, StringLengthPtr));
end;

procedure TODBCCustomStatement.SetAttribute(const Attribute: SQLINTEGER; ValuePtr: SQLPOINTER; StringLength: SQLINTEGER);
begin
  CheckPrepared;
  CheckError(SQLSetStmtAttr(FStmtHandle, Attribute, ValuePtr, StringLength));
end;
  ...
  begin
    repeat
      ...
      begin
        ...
        if FParams[ParamIndex].CType = SQL_C_WCHAR then
          ParamLength := FParams[ParamIndex].DataLength * SizeOf(WideChar)
        else
