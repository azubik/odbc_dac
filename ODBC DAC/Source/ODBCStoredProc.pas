...
begin
  ...
  try
    RetCode := SQLProcedureColumns(TmpHandle, nil, 0, nil, 0, PSQLCHAR(FStoredProcName), SQL_NTS, nil, 0);
    if not SQL_SUCCEEDED(RetCode) then
      ODBCError(SQL_HANDLE_STMT, TmpHandle, RetCode);

    RetCode := SQLBindCol(TmpHandle, 4, SQL_C_CHAR, @ColName, STR_LEN, @StrLen_or_Ind);
    if not SQL_SUCCEEDED(RetCode) then
      ODBCError(SQL_HANDLE_STMT, TmpHandle, RetCode);
