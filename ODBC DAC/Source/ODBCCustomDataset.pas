...
const
  ...
  FieldIsNull = #0;                               // FieldIsNull flag
  FieldIsNotNull = #1;                            // FieldIsNotNull flag
...
// |<-------------------- FRecBufSize --------------------->|
  ...
  private
    ...
    procedure FreeBlobFields(Buffer: PChar);

    procedure FetchNext(Buffer: PChar);
    function WriteBufferToCache(ARecNum: Integer; Buffer: PChar): PChar;

    procedure CopyBuffer(Source: PChar; Dest: PChar);
    procedure SaveToOldCache(Buffer: PChar);

    function GetCacheBuffer(ABuffer: PChar): PChar;
    function GetCacheBufferA(ARecNum: Integer): PChar;
    function GetCacheOldBuffer(ARecNum: Integer): PChar;

    function AdjustCurrentRecord(Buffer: PChar; GetMode: TGetMode): TGetResult;
    function IsVisible(Buffer: PChar): Boolean;

    // other methods
    procedure RefreshParams;

    procedure GetBlobData(Field: TField; Buffer: PChar; out Data: Pointer; out DataLength: Integer);
    procedure SetBlobData(Field: TField; Buffer: PChar; Data: Pointer; DataLength: Integer);

    procedure AddToUpdateHistory(Buffer: PChar);
    procedure RemoveFromUpdateHistory(Buffer: PChar);
  ...
  protected
    ...
    function GetFieldDefSize(FT: TFieldType; Len: Integer; DecDigits: SmallInt): Integer;
    ...
    procedure ApplyCachedUpdate(Buffer: PChar; var UpdateAction: TUpdateAction); virtual;
    procedure RevertCachedUpdate(Buffer: PChar);
    ...
    property _CurrentChunk: PChar read FCurrentChunk write FCurrentChunk;

    property _CacheList: TList read FCacheList write FCacheList;

    property _OldCache: PChar read FOldCache write FOldCache;
    ...
    property _FilterBuffer: PChar read FFilterBuffer write FFilterBuffer;
    property _OldBuffer: PChar read FOldBuffer write FOldBuffer;
...
{$ENDIF}
  ...
  private
    ...
    procedure SetInternalSQLParams(Stmt: TODBCCustomStatement; Buffer: PChar);
  ...
  protected
    ...
    procedure ApplyCachedUpdate(Buffer: PChar; var UpdateAction: TUpdateAction); override;
...
procedure TODBCCustomDataSet.FreeBlobFields(Buffer: PChar);
var
  i: Integer;
begin
...
procedure TODBCCustomDataSet.FetchNext(Buffer: PChar);
var
  Fld: TField;
...
begin
  ...
  if FBlobBufferSize > 0 then
    FillChar(PChar(Buffer + FBlobCacheOfs)^, FBlobBufferSize, 0);
    ...
    begin
      ...
      case StmtField.SQLType of
        SQL_CHAR, SQL_VARCHAR,
          ...
          StrLCopy(@Buffer[FieldDesc.DataOfs], StmtField.Data, StmtField.DataLength {FieldDesc^.DataSize});

        SQL_WCHAR, SQL_WVARCHAR:
          Move(PWideChar(StmtField.Data)^, Buffer[FieldDesc.DataOfs], FieldDesc.DataSize {StmtField.DataLength});
        ...
        SQL_BINARY, SQL_VARBINARY, SQL_LONGVARBINARY, SQL_LONGVARCHAR:
          if FCacheBlobs then
          begin
            PBlobDataArray(Buffer + FBlobCacheOfs)^[FieldOffset] := StmtField.AsString;
            Inc(FieldOffset);
          end;
        ...
        SQL_WLONGVARCHAR:
          ...
          begin
            ...
            Move(StmtField.Data^, PChar(PBlobDataArray(Buffer + FBlobCacheOfs)^[FieldOffset])^, StmtField.DataLength);

            Inc(FieldOffset);
          end;

        SQL_GUID:
          StrLCopy(@Buffer[FieldDesc.DataOfs], PChar(StmtField.AsString), Length(StmtField.AsString));
      else
...
function TODBCCustomDataSet.WriteBufferToCache(ARecNum: Integer; Buffer: PChar): PChar;
var
  Index: Integer;
begin
...
procedure TODBCCustomDataSet.CopyBuffer(Source: PChar; Dest: PChar);
var
  i: Integer;
begin
  FreeBlobFields(Dest);
...
procedure TODBCCustomDataSet.SaveToOldCache(Buffer: PChar);
var
...
function TODBCCustomDataSet.GetCacheBuffer(ABuffer: PChar): PChar;
begin
  Result := GetCacheBufferA(PRecInfo(ABuffer)^.PhyRecNum);
end;

function TODBCCustomDataSet.GetCacheBufferA(ARecNum: Integer): PChar;
var
  i: Integer;
begin
  i := 0;
...
function TODBCCustomDataSet.GetCacheOldBuffer(ARecNum: Integer): PChar;
var
  Size: Integer;
  Offset: Integer;
begin
...
function TODBCCustomDataSet.AdjustCurrentRecord(Buffer: PChar; GetMode: TGetMode): TGetResult;
begin
  Result := grOK;
...
function TODBCCustomDataSet.IsVisible(Buffer: PChar): Boolean;
begin
  Result := True;
...
procedure TODBCCustomDataSet.GetBlobData(Field: TField; Buffer: PChar; out Data: Pointer; out DataLength: Integer);
begin
  Data := PChar(PBlobDataArray(Buffer + FBlobCacheOfs)^[Field.Offset]);
  DataLength := Length(PBlobDataArray(Buffer + FBlobCacheOfs)^[Field.Offset]);
end;

procedure TODBCCustomDataSet.SetBlobData(Field: TField; Buffer: PChar; Data: Pointer; DataLength: Integer);
begin
  if Buffer <> ActiveBuffer then Exit;

  SetLength(PBlobDataArray(Buffer + FBlobCacheOfs)^[Field.Offset], DataLength);
  Move(Data^, PChar(PBlobDataArray(Buffer + FBlobCacheOfs)^[Field.Offset])^, DataLength);
end;

procedure TODBCCustomDataSet.AddToUpdateHistory(Buffer: PChar);
begin
  if FUpdateHistory.IndexOf(Buffer) = -1 then
    FUpdateHistory.Add(Buffer);
end;

procedure TODBCCustomDataSet.RemoveFromUpdateHistory(Buffer: PChar);
begin
  FUpdateHistory.Remove(Buffer);
end;

function TODBCCustomDataSet.GetFieldClass(FieldType: TFieldType): TFieldClass;
begin
  case FieldType of
    ftWideString:
      Result := TsvWideStringField;
    ftFmtMemo:
      Result := TsvWideMemoField;
  else
    Result := inherited GetFieldClass(FieldType);
  end;
end;

procedure TODBCCustomDataSet.DataConvert(Field: TField; Source, Dest: Pointer; ToNative: Boolean);
begin
  if Field.DataType = ftWideString then
  begin
    if ToNative then
      //PWideString -> PWideChar
      Move(PWideChar(WideString(Source^))^, Dest^, 2 * Length(WideString(Source^)) + SizeOf(WideChar))
    else
      // PWideChar -> PWideString
...
function TODBCCustomDataSet.GetFieldDefSize(FT: TFieldType; Len: Integer; DecDigits: SmallInt): Integer;
begin
  case FT of
...
function TODBCCustomDataSet.GetActiveBuffer: PChar;
begin
  Result := nil;
...
procedure TODBCCustomDataSet.ApplyCachedUpdate(Buffer: PChar; var UpdateAction: TUpdateAction);
var
...
procedure TODBCCustomDataSet.RevertCachedUpdate(Buffer: PChar);
var
  OldBuff: PChar;
begin
...
function TODBCCustomDataSet.AllocRecordBuffer: PChar;
begin
  Result := AllocMem(FRecBufSize);
end;

procedure TODBCCustomDataSet.FreeRecordBuffer(var Buffer: PChar);
begin
  FreeMem(Buffer);
end;

procedure TODBCCustomDataSet.InternalInitRecord(Buffer: PChar);
begin
...
function TODBCCustomDataSet.InternalGetRecord(Buffer: PChar; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
begin
  Result := grOK;
...
function TODBCCustomDataSet.GetRecord(Buffer: PChar; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
var
  Accept: Boolean;
...
function TODBCCustomDataSet.GetBookmarkFlag(Buffer: PChar): TBookmarkFlag;
begin
  Result := PRecInfo(Buffer)^.BookmarkFlag;
end;

procedure TODBCCustomDataSet.SetBookmarkFlag(Buffer: PChar; Value: TBookmarkFlag);
begin
  PRecInfo(Buffer)^.BookmarkFlag := Value;
end;

procedure TODBCCustomDataSet.GetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  PSQLINTEGER(Data)^ := PRecInfo(Buffer)^.PhyRecNum;
end;

procedure TODBCCustomDataSet.SetBookmarkData(Buffer: PChar; Data: Pointer);
begin
...
procedure TODBCCustomDataSet.InternalSetToRecord(Buffer: PChar);
begin
...
begin
  ...
  try
    ...
    begin
      ...
      FieldDef.Size := GetFieldDefSize(FieldDef.DataType, StmtField.DataLength, StmtField.DecimalDigits);
...
procedure TODBCCustomDataSet.ClearCalcFields(Buffer: PChar);
begin
...
function TODBCCustomDataSet.GetCurrentRecord(Buffer: PChar): Boolean;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Data := PChar(Buff + FMetaFields^[Field.FieldNo].DataOfs);
...
procedure TODBCInternalDataSet.SetInternalSQLParams(Stmt: TODBCCustomStatement; Buffer: PChar);
var
  Param: TODBCParam;
  ParamName: string;
  TmpField: TField;
...
procedure TODBCInternalDataSet.ApplyCachedUpdate(Buffer: PChar; var UpdateAction: TUpdateAction);
var
  UpdateKind: TUpdateKind;
begin
