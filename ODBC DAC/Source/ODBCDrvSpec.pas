...
begin
  // Get Driver Name
  RetCode := SQLGetInfo(ConnHandle, SQL_DRIVER_NAME, @Buffer, STR_LEN, @BufLen);
  if not SQL_SUCCEEDED(RetCode) then
    ODBCError(SQL_HANDLE_DBC, ConnHandle, RetCode);

  UC_DriverName := UpperCase(Buffer);
...
begin
  // Get DBMS Name
  RetCode := SQLGetInfo(ConnHandle, SQL_DBMS_NAME, @Buffer, STR_LEN, @BufLen);
  if not SQL_SUCCEEDED(RetCode) then
    ODBCError(SQL_HANDLE_DBC, ConnHandle, RetCode);

  UC_DBMSName := UpperCase(Buffer);
  ...
  if UC_DBMSName = 'SQLBASE' then
    ...
    Result := dbmstIbmDb2
  else if (StrLComp(PAnsiChar(UC_DBMSName), 'MYSQL', 5) = 0) then
    Result := dbmstMySql
    ...
    Result := dbmstText
  else if (StrLComp(PAnsiChar(UC_DBMSName), 'DBASE', 5) = 0) then // DBASE II, IV, V
    Result := dbmstDBase
  ...
  else if (StrLComp(PAnsiChar(UC_DBMSName), 'FIREBIRD', 8) = 0) then // 'FIREBIRD / INTERBASE(R)'
    Result := dbmstInterbase
    ...
    Result := dbmstSQLLite
  else if (StrLComp(PAnsiChar(UC_DBMSName), 'THINKSQL', 8) = 0) then // 'THINKSQL RELATIONAL DATABASE MANAGEMENT SYSTEM'
  ...
  else if (StrLComp(PAnsiChar(UC_DBMSName), 'POSTGRESQL', 10) = 0) then
